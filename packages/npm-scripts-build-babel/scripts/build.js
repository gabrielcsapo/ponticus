  ...
  try {
    if (fs.statSync("./.npmscriptrc").isFile()) {
      configInfo = JSON.parse(
        stripJsonComments(fs.readFileSync("./.npmscriptrc", "utf-8"))
      );
    }
  } catch (err) {
    /* nop */
...
if (typeof configInfo !== "object") {
  throw new Error(
    "TyphonJS NPM script (build-babel) could not load `./npmscriptrc.js` or `./npmscriptrc`."
  );
}
...
if (typeof configInfo[buildEntry] !== "object") {
  throw new Error(
    "typhonjs-npm-scripts-build-babel error: '" +
      buildEntry +
...
if (typeof babelConfig.source !== "string") {
  throw new Error(
    "typhonjs-npm-scripts-build-babel error: '" +
      buildEntry +
      ".babel.source' entry is not a string or is missing in " +
      "config file."
  );
}
...
try {
  if (!fs.statSync(babelConfig.source).isDirectory()) {
    throw new Error(
      "'" +
        buildEntry +
        ".babel.source' entry is not a directory: " +
        babelConfig.source
    );
  }
} catch (err) {
...
try {
  if (!fs.statSync(babelConfig.destination).isDirectory()) {
    throw new Error(
      "'" +
        buildEntry +
        "babel.destination' entry is not a directory: " +
        babelConfig.destination
    );
  }
} catch (err) {
  throw new Error("typhonjs-npm-scripts-build-babel error: " + err);
...
if (Array.isArray(configInfo[buildEntry].copy)) {
  for (cntr = 0; cntr < configInfo[buildEntry].copy.length; cntr++) {
    ...
    /* istanbul ignore if */
    if (
      typeof copyEntry.source !== "string" ||
      typeof copyEntry.destination !== "string"
    ) {
      throw new Error(
        "typhonjs-npm-scripts-build-babel error: '" +
          buildEntry +
    ...
    console.log(
      "typhonjs-npm-scripts-build-babel copy: " +
        copyEntry.source +
        " " +
...
if (Array.isArray(configInfo[buildEntry].chmod)) {
  for (cntr = 0; cntr < configInfo[buildEntry].chmod.length; cntr++) {
    ...
    /* istanbul ignore if */
    if (
      typeof chmodEntry.path !== "string" ||
      typeof chmodEntry.mode !== "string"
    ) {
      throw new Error(
        "typhonjs-npm-scripts-build-babel error: '" +
          buildEntry +
    ...
    console.log(
      "typhonjs-npm-scripts-build-babel chmod: " +
        chmodEntry.path +
        " " +
        chmodEntry.mode
    );
...
if (files.length === 0) {
  throw new Error(
    "typhonjs-npm-scripts-build-babel error: empty destination directory: " +
      babelConfig.destination
  );
}
