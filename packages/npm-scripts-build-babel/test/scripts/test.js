...
function verifyOutput(distDir) {
  ...
  if ("Test Success" !== new TestDummy().test()) {
    throw new Error(
      "typhonjs-npm-scripts-build-babel test error: transpiling failed."
    );
  }
  ...
  try {
    if (!fs.statSync("./test/" + distDir + "/subdir/test2.json").isFile()) {
      throw new Error(
        "'./test/" + distDir + "/subdir/test2.json' not found. (copy failed)"
      );
    }
  } catch (err) {
    throw new Error("typhonjs-npm-scripts-build-babel test error: " + err);
  ...
  try {
    if (!fs.statSync("./test/" + distDir + "/test.json").isFile()) {
      throw new Error(
        "'./test/" + distDir + "/test.json' not found. (script failed)"
      );
    }
  } catch (err) {
    throw new Error("typhonjs-npm-scripts-build-babel test error: " + err);
  ...
  try {
    var utilResult = util.inspect(
      fs.statSync("./test/" + distDir + "/TestDummy.js")
    );
