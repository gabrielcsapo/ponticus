...
export default class ClassReport extends AbstractReport {
  ...
  getErrors(options = { includeChildren: true, includeReports: false }) {
    ...
    let errors = options.includeReports
  ...
  static parse(object) {
    ...
    if (classReport.errors.length > 0) {
      classReport.errors = classReport.errors.map((error) =>
        AnalyzeError.parse(error)
      );
    }

    if (classReport.methods.length > 0) {
      classReport.methods = classReport.methods.map((methodReport) =>
