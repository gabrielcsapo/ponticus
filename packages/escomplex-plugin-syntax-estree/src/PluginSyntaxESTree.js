...
export default class PluginSyntaxESTree extends AbstractSyntaxLoader {
  ...
  /**
    ...
    */
  onConfigure(ev) {
    ev.data.settings.commonjs =
      typeof ev.data.options.commonjs === "boolean"
        ? ev.data.options.commonjs
        : false;

    ev.data.settings.dependencyResolver =
      typeof ev.data.options.dependencyResolver === "function"
        ? ev.data.options.dependencyResolver
        : void 0;

    if (typeof ev.data.options.esmImportExport === "object") {
      ev.data.settings.esmImportExport = {
        halstead:
          typeof ev.data.options.esmImportExport.halstead === "boolean"
            ? ev.data.options.esmImportExport.halstead
            : false,

        lloc:
          typeof ev.data.options.esmImportExport.lloc === "boolean"
            ? ev.data.options.esmImportExport.lloc
            : false,
      };
    } else {
      // Catch all boolean to set `esmImportExport.halstead` & `esmImportExport.lloc` if a boolean is supplied.
      const value =
        typeof ev.data.options.esmImportExport === "boolean"
          ? ev.data.options.esmImportExport
          : false;
      ev.data.settings.esmImportExport = { halstead: value, lloc: value };
    }

    ev.data.settings.forin =
      typeof ev.data.options.forin === "boolean"
        ? ev.data.options.forin
        : false;

    ev.data.settings.logicalor =
      typeof ev.data.options.logicalor === "boolean"
        ? ev.data.options.logicalor
        : true;

    ev.data.settings.switchcase =
      typeof ev.data.options.switchcase === "boolean"
        ? ev.data.options.switchcase
        : true;

    if (typeof ev.data.options.templateExpression === "object") {
      ev.data.settings.templateExpression = {
        halstead:
          typeof ev.data.options.templateExpression.halstead === "boolean"
            ? ev.data.options.templateExpression.halstead
            : true,

        lloc:
          typeof ev.data.options.templateExpression.lloc === "boolean"
            ? ev.data.options.templateExpression.lloc
            : true,
      };
    } else {
      // Catch all boolean to set `templateExpression.halstead` & `templateExpression.lloc` if a boolean is supplied.
      const value =
        typeof ev.data.options.templateExpression === "boolean"
          ? ev.data.options.templateExpression
          : true;

      ev.data.settings.templateExpression = { halstead: value, lloc: value };
    }

    ev.data.settings.trycatch =
      typeof ev.data.options.trycatch === "boolean"
        ? ev.data.options.trycatch
        : false;
  }
    ...
    return actualize(
      ...
      (node) => {
        ...
        if (
          ...
          if (
            dependency.type === "Literal" ||
            dependency.type === "StringLiteral"
          ) {
            dependencyPath =
              typeof settings.dependencyResolver === "function"
                ? settings.dependencyResolver(dependency.value)
                : dependency.value;
          }
    ...
    return actualize(
      ...
      (node) => {
        ...
        if (childNodes.length > 0) {
          operators.push(...ASTGenerator.parseNodes(childNodes).operators);
        }
      ...
      (node) => {
        ...
        if (childNodes.length > 0) {
          operands.push(...ASTGenerator.parseNodes(childNodes).operands);
        }
    ...
    return actualize(
      ...
      (node, parent) => {
        ...
        if (parent && parent.type === "MethodDefinition") {
          ...
          operators.push(
            typeof node.generator === "boolean" && node.generator
              ? "function*"
              : "function"
          );
        }
  ...
  Property() {
    return actualize(1, 0, (node) => {
      return typeof node.shorthand === "undefined"
        ? ":"
        : typeof node.shorthand === "boolean" && !node.shorthand
        ? ":"
        : void 0;
    });
  }
    ...
    return actualize(
      ...
      (node, parent) => {
        ...
        return {
          ...
          operators: isValid
            ? new HalsteadArray("operators", operators)
            : void 0,
          paramNames,
      ...
      if (typeof node.superClass !== "undefined" && node.superClass !== null) {
        ...
        superClassName =
          node.superClass.type === "Identifier"
            ? TraitUtil.safeName(node.superClass)
  ...
  ExportAllDeclaration(settings) {
    return actualize(
      settings.esmImportExport.lloc ? 1 : 0,
      0,
      settings.esmImportExport.halstead ? ["export", "*"] : void 0,
      void 0,
  ...
  ExportDefaultDeclaration(settings) {
    return actualize(
      settings.esmImportExport.lloc ? 1 : 0,
      0,
      settings.esmImportExport.halstead ? ["export", "default"] : void 0
    );
  }
  ...
  ExportNamedDeclaration(settings) {
    return actualize(
      ...
      settings.esmImportExport.halstead ? ["export", "{}"] : void 0,
      void 0,
      settings.esmImportExport.halstead ? void 0 : ["source", "specifiers"]
    );
  }
  ...
  ExportSpecifier(settings) {
    return actualize(
      0,
      0,
      !settings.esmImportExport.halstead
        ? void 0
        : (node) => {
  ...
  ImportDeclaration(settings) {
    return actualize(
      settings.esmImportExport.lloc ? 1 : 0,
      0,
      settings.esmImportExport.halstead ? ["import", "from"] : void 0,
      void 0,
      settings.esmImportExport.halstead ? void 0 : null,
      void 0,
      (node) => {
        const dependencyPath =
          typeof settings.dependencyResolver === "function"
            ? settings.dependencyResolver(node.source.value)
            : node.source.value;
  ...
  ImportNamespaceSpecifier(settings) {
    return actualize(
      0,
      0,
      settings.esmImportExport.halstead ? ["import", "*", "as"] : void 0,
      void 0,
      settings.esmImportExport.halstead ? void 0 : null
    );
  }
  ...
  MetaProperty() {
    return actualize(0, 0, ".", (node) => {
      return typeof node.meta === "string" && typeof node.property === "string"
        ? [node.meta, node.property]
        : void 0;
    });
  }
   ...
   * @returns {{lloc: *, cyclomatic: *, operators: *, operands: *, ignoreKeys: *, newScope: *, dependencies: *}}
   */
  TaggedTemplateExpression(settings) {
    return actualize(settings.templateExpression.lloc ? 1 : 0, 0);
  }
  ...
  TemplateLiteral(settings) {
    return actualize(
      0,
      0,
      !settings.templateExpression.halstead
        ? []
        : (node) => {
            const operators = ["``"];
  ...
  YieldExpression() {
    return actualize(
      ...
      0,
      (node) => {
        return typeof node.delegate === "boolean" && node.delegate
          ? "yield*"
          : "yield";
      }
    );
    ...
    if (typeof parent.computed === "boolean" && parent.computed) {
      ...
      name =
        parent.key.type === "Literal"
          ? TraitUtil.safeValue(parent.key)
...
function s_SAFE_COMPUTED_OPERANDS(node, parent) {
  ...
  if (
    operands.length === 0 &&
    ((typeof node.id !== "undefined" && node.id !== null) ||
      (typeof node.key !== "undefined" && node.key !== null))
  ) {
