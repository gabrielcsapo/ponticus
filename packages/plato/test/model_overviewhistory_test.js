...
exports["history"] = {
  ...
  "test new historical report generation for overview": function (test) {
    ...
    test.equal(
      history[0].total.sloc,
      newReport.summary.total.sloc,
      "Should have added values into an array"
    );
    test.equal(
      history[0].total.maintainability,
      newReport.summary.total.maintainability,
      "Should have added values into an array"
    );
    test.equal(
      history[0].average.sloc,
      newReport.summary.average.sloc,
      "Should have added values into an array"
    );
    test.equal(
      history[0].average.maintainability,
      newReport.summary.average.maintainability,
      "Should have added values into an array"
    );
    test.done();
  },
  "test history generation from existing history": function (test) {
    ...
    test.deepEqual(
      history.total,
      data.total,
      "Should populate itself properly"
    );
    test.deepEqual(
      history.average,
      data.average,
