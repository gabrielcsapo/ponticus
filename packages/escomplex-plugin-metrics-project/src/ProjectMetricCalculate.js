import MathUtil from "@ponticus/escomplex-commons/dist/utils/MathUtil";
...
export default class ProjectMetricCalculate {
  ...
  static calculate(projectReport, pathModule, settings) {
    const adjacencyMatrix = ProjectMetricCalculate.calculateAdjacencyMatrix(
      projectReport,
      pathModule
    );

    if (!settings.noCoreSize) {
      const visibilityMatrix = ProjectMetricCalculate.calculateVisibilityMatrix(
        projectReport,
        adjacencyMatrix
      );
  ...
  static calculateAdjacencyMatrix(projectReport, pathModule) {
    ...
    for (let x = 0; x < length; x++) {
      for (let y = 0; y < length; y++) {
        adjacencyMatrix[x][y] =
          x !== y &&
          ProjectMetricCalculate.doesDependencyExist(
            pathModule,
            modules[x],
            modules[y]
          )
    ...
    projectReport.firstOrderDensity = MathUtil.getPercent(
      density,
      length * length
    );
  ...
  static calculateCoreSize(projectReport, visibilityMatrix) {
    ...
    for (let rowIndex = 0; rowIndex < length; rowIndex++) {
      fanIn[rowIndex] = visibilityMatrix[rowIndex].reduce(
        (sum, value, valueIndex) => {
          fanOut[valueIndex] =
            rowIndex === 0 ? value : fanOut[valueIndex] + value;
          return sum + value;
        },
        0
      );
    ...
    for (let rowIndex = 0; rowIndex < length; rowIndex++) {
      if (
        fanIn[rowIndex] >= boundaries.fanIn &&
        fanOut[rowIndex] >= boundaries.fanOut
      ) {
        coreSize += 1;
      }
    }
  ...
  static calculateVisibilityMatrix(projectReport, adjacencyMatrix) {
    ...
    for (let x = 0; x < length; x++) {
      for (let y = 0; y < length; y++) {
        visibilityMatrix[x][y] =
          x === y ? 1 : adjacencyMatrix[x][y] || Infinity;
      }
    }
    ...
    for (let k = 0; k < length; k++) {
      for (let x = 0; x < length; x++) {
        for (let y = 0; y < length; y++) {
          if (
            visibilityMatrix[x][y] >
            visibilityMatrix[x][k] + visibilityMatrix[k][y]
          ) {
            visibilityMatrix[x][y] =
              visibilityMatrix[x][k] + visibilityMatrix[k][y];
          }
  ...
  static doesDependencyExist(pathModule, fromModuleReport, toModuleReport) {
    ...
    for (let cntr = 0; cntr < fromModuleReport.dependencies.length; cntr++) {
      ...
      if (
        typeof toModuleReport.srcPathAlias === "string" &&
        depPath === toModuleReport.srcPathAlias
      ) {
    ...
    for (let cntr = 0; cntr < fromModuleReport.dependencies.length; cntr++) {
      ...
      if (pathModule.extname(depPath) === "") {
        depPath += pathModule.extname(toModuleReport.srcPath);
      }
      ...
      if (
        pathModule.resolve(fromModuleReport_dirname, depPath) ===
        toModuleReport.srcPath
      ) {
        matchedDependency = true;
        break;
      }
