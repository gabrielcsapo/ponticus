...
export default class ModuleMetricPostAverage {
  ...
  static calculate(moduleReport, settings) {
    ...
    ModuleMetricPostAverage.calculateMaintainabilityIndex(
      moduleReport,
      settings,
      moduleAggregateAverage.cyclomatic,
      moduleAggregateAverage.halstead.effort,
      moduleAggregateAverage.sloc.logical
    );
    ...
    moduleReport.classes.forEach((classReport) => {
      ...
      ModuleMetricPostAverage.calculateMaintainabilityIndex(
        classReport,
        settings,
        classMethodAverages.cyclomatic,
        classMethodAverages.halstead.effort,
        classMethodAverages.sloc.logical
      );
    });
  }
  ...
  static calculateMaintainabilityIndex(
    ...
    /* istanbul ignore if */
    if (settings.newmi) {
      report.maintainability = Math.max(
        0,
        (report.maintainability * 100) / 171
      );
    }
  }
}
