...
export default class ModuleMetricCalculate {
  ...
  static calculate(moduleReport) {
    ...
    moduleReport.classes.forEach((classReport) => {
      ...
      ModuleMetricCalculate.calculateCyclomaticDensity(
        classReport.aggregateReport
      );
      ModuleMetricCalculate.calculateHalsteadMetrics(
        classReport.aggregateReport.halstead
      );
    });
    ...
    ModuleMetricCalculate.calculateHalsteadMetrics(
      moduleReport.aggregateReport.halstead
    );
  }
  ...
  static calculateCyclomaticDensity(report) {
    report.cyclomaticDensity =
      report.sloc.logical === 0
        ? 0
        : (report.cyclomatic / report.sloc.logical) * 100;
  }
  ...
  static calculateHalsteadMetrics(halstead) {
    ...
    if (halstead.length === 0) {
      ...
      halstead.vocabulary =
        halstead.operators.distinct + halstead.operands.distinct;
      halstead.difficulty =
        (halstead.operators.distinct / 2) *
        (halstead.operands.distinct === 0
          ? 1
          : halstead.operands.total / halstead.operands.distinct);
      halstead.volume =
        halstead.length * (Math.log(halstead.vocabulary) / Math.log(2));
