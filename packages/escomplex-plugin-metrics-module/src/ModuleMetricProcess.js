import { HalsteadArray } from "@ponticus/escomplex-commons";
...
export default class ModuleMetricProcess {
  ...
  static addDependencies(moduleReport, dependencies) {
    if (typeof dependencies === "object" || Array.isArray(dependencies)) {
      moduleReport.dependencies =
        moduleReport.dependencies.concat(dependencies);
    }
  }
  ...
  static preScopeCreated(
    ...
    if (typeof newScope !== "object") {
      throw new TypeError(
        `preScopeCreated error: 'newScope' is not an 'object'.`
      );
    }

    if (typeof newScope.type !== "string") {
      throw new TypeError(
        `preScopeCreated error: 'newScope.type' is not a 'string'.`
      );
    }
      ...
      case "method":
        ...
        if (newScope.operands instanceof HalsteadArray) {
          ...
          identifiers.forEach((identifier) => {
            ModuleMetricProcess.halsteadItemEncountered(
              moduleReport,
              scopeControl,
              "operands",
              identifier
  ...
  static postScopeCreated(moduleReport, scopeControl, newScope = {}) {
    if (typeof newScope !== "object") {
      throw new TypeError(
        `postScopeCreated error: 'newScope' is not an 'object'.`
      );
    }

    if (typeof newScope.type !== "string") {
      throw new TypeError(
        `postScopeCreated error: 'newScope.type' is not a 'string'.`
      );
    }
    ...
    switch (newScope.type) {
      ...
      case "method": {
        if (!Number.isInteger(newScope.cyclomatic)) {
          throw new TypeError(
            `postScopeCreated error: 'newScope.cyclomatic' is not an 'integer'.`
          );
        }

        if (!Array.isArray(newScope.paramNames)) {
          throw new TypeError(
            `postScopeCreated error: 'newScope.paramNames' is not an 'array'.`
          );
        }
   ...
   */
  static halsteadItemEncountered(
    moduleReport,
    scopeControl,
    metric,
    identifier
    ...
    ModuleMetricProcess.incrementHalsteadItems(
      moduleReport,
      metric,
      identifier
    );
  ...
  static incrementHalsteadItems(report, metric, identifier) {
    ...
    if (
      report.aggregateReport.halstead[metric].identifiers.indexOf(
        identifier
      ) === -1
    ) {
  ...
  static processSyntax(moduleReport, scopeControl, syntax, node, parent) {
    for (const key in syntax) {
      ...
      switch (trait.metric) {
        case "cyclomatic":
          ModuleMetricProcess.incrementCyclomatic(
            moduleReport,
            scopeControl,
            trait.valueOf(node, parent)
          );
          break;

        case "dependencies":
          ModuleMetricProcess.addDependencies(
            moduleReport,
            trait.valueOf(node, parent)
          );
          break;
        ...
        case "lloc":
          ...
          // }
          ModuleMetricProcess.incrementLogicalSloc(
            moduleReport,
            scopeControl,
            trait.valueOf(node, parent)
          );
