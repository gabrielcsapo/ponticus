import ObjectUtil from "@ponticus/escomplex-commons/dist/utils/ObjectUtil";
...
export default class ModuleMetricAverage {
  ...
  static calculate(moduleReport) {
    ...
    moduleReport.methods.forEach((methodReport) => {
      ObjectUtil.safeBatchSet(
        moduleReport.methodAverage,
        moduleReport.methodAverage.keys,
        methodReport,
        "add"
      );
    });
    ...
    moduleReport.classes.forEach((classReport) => {
      ...
      classReport.methods.forEach((methodReport) => {
        ObjectUtil.safeBatchSet(
          moduleReport.methodAverage,
          classReport.methodAverage.keys,
          methodReport,
          "add"
        );
        ObjectUtil.safeBatchSet(
          classReport.methodAverage,
          classReport.methodAverage.keys,
          methodReport,
          "add"
        );
      ...
      ObjectUtil.safeBatchSet(
        classReport.aggregateAverage,
        classReport.aggregateAverage.keys,
        classReport.aggregate,
        "add"
      );
    ...
    ObjectUtil.safeBatchSet(
      moduleReport.aggregateAverage,
      moduleReport.aggregateAverage.keys,
      moduleReport.aggregate,
      "add"
    );
